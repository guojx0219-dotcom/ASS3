...
public abstract class Mob extends Char {
	...
	protected void onAdd(){
		if (firstAdded) {
			...
			MobLogger.logSpawn(id(), getClass().getSimpleName(), pos);
			firstAdded = false;
		}
	}
	...
	protected boolean act() {
		...
		Char prevEnemy = enemy;
		enemy = chooseEnemy();
		if (enemy != null && enemy != prevEnemy) {
			MobLogger.logTargetChange(id(), getClass().getSimpleName(),
					enemy.id(), enemy.getClass().getSimpleName());
		}
	...
	protected void setState(AiState newState) {
		...
		String oldName = stateName(state);
		state = newState;
		MobLogger.logStateChange(id(), getClass().getSimpleName(), oldName, stateName(newState));
	}
	...
	private String stateName(AiState s) {
		...
		return s.getClass().getSimpleName().toUpperCase();
	}

	public void aggro( Char ch ) {
		Char prevEnemy = enemy;
		enemy = ch;
		if (ch != null && ch != prevEnemy) {
			MobLogger.logTargetChange(id(), getClass().getSimpleName(),
					ch.id(), ch.getClass().getSimpleName());
		}
		if (state != PASSIVE){
			setState(HUNTING);
		}
	...
	public void clearEnemy(){
		enemy = null;
		enemySeen = false;
		if (state == HUNTING) setState(WANDERING);
	}
